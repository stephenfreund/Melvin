...
def model_table(raw: Dict[str, str], funcs: Optional[Dict[str, List]] = None,
                class_fields: Optional[Dict[str, Dict]] = None) -> List[Tuple[str, str]]:
        ...
        for raw in out.splitlines():
            ...
            if stripped == "*** END_MODEL":
                if model_lines is not None:
                    try:
                        entries, funcs = _parse_model_block(model_lines)
                        pending_model = model_table(entries, funcs, class_fields)
