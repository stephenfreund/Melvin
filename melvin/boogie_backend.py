...
def model_table(raw: Dict[str, str],
    ...
    functions that do not actually use them.  In-scope variables the model
    does not constrain at all (Boogie prunes SSA incarnations irrelevant to
    the failing path) are listed with the value `?` so the omission is
